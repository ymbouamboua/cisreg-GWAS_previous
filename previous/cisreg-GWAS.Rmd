...
parameters <- list(
  ...
  #maf <- 0.05,
  r2 <- 0.8,
  working.dir = working.dir <- '~/Google\ Drive/Yvon_Mbouamboua_these_2018/gwas/cisreg-GWAS',
...
We used the [halpoR package](https://cran.r-project.org/web/packages/haploR/vignettes/haplor-vignette.html) [@ward_haploreg:_2012; @ward_haploreg_2016] which allows to recover SNPs in high LD (with the **r^2^ = 0.8**) using HaploReg by specifying the population. We specified the African population and submitted the **Tag SNPs from GWAS catalog.**
  ...
  **
  To do: tableau avec 1 ligne par SNP associé à la maladie, et des colonnes d'information sur les haplotypes
- start
- end
- nb of SNPs
**
...
```{r}
haplotype <- SOIs[, c("chr", "pos_hg38", "rsID", "query_snp_rsid")]
haplotype <- na.omit(haplotype)
haplotype$pos_hg38 <- as.numeric(haplotype$pos_hg38)
...
tata.file <- "tata.tsv"
tata.path <- file.path(dir.path["SOIs"], tata.file)
write.table(x = tata,
            file = tata.path,
            quote = FALSE,
            sep = "\t",
...
```{r tf_jaspar2018}
message("Downloading matrices from JASPAR")

# Download all matrice from JASPAR
jaspar.url <- "http://hfaistos.uio.no:8000/download/CORE/JASPAR2018_CORE_vertebrates_non-redundant_pfms_jaspar.txt"

jaspar.file <- paste(sep = "JASPAR2018_CORE_vertebrates_NR", 'data/','.tsv')

if (parameters$force.download || !file.exists(jaspar.file)) {
  message("Downloading disease-associated SNPs and Genes from GWAS catalog")
  download.file(url = jaspar.url,
                destfile = jaspar.file, method = 'auto')
  message("\tDownloaded , JASPAR2018_CORE_vertebrates_NR file in ", 'data/')
} else {
  message("\tJASPAR2018_CORE_vertebrates_NR file already there: ", jaspar.file)
}

## Loading matrix data
matrices <- read.delim(file = "./data/data.tsv", 
                       header = FALSE, 
                       sep = "\t", 
                       stringsAsFactors = FALSE,
                       na.strings = c(""," ","NA"))

#matrices$V1 <- gsub(pattern = ">", " ", matrices$V1)

#View(matrices)
## Select motifs and transcription factor names
jaspar.core.nr.2018 <- dplyr::filter(matrices, grepl('^>', matrices[,1]))

## Delete the caractor > in the first column 
jaspar.core.nr.2018$V1 <- gsub(">", " ", jaspar.core.nr.2018$V1)
jaspar.core.nr.2018 <- as.data.frame(jaspar.core.nr.2018)
#names(jaspar.core.nr.2018) <- c("X.ac_motif", "TF")
#View(jaspar.core.nr.2018)
...
remap.inter.SOIs <- data.frame(IntersectBed(remapCatalog, SOIsGR)) 
...
varscan.file <- "results/RSAT/Mycobacterium_tuberculosis_data-variation-scan_vs_jaspar_nonred.tsv"
varScanJaspar <- read.delim(file = varscan.file,
                            header = TRUE,
                            sep = "\t",
                            stringsAsFactors = FALSE,
                            na.strings = c(""," ","NA"))
...
# Upload variation-scan negative control (negTuberculosis4)
varScanJaspar.file <- 'results/RSAT/SOIs_varscan_neg.tsv'
varScanJasparNeg <- read.delim(file = varScanJaspar.file,
                               header = TRUE,
                               sep = "\t",
                               stringsAsFactors = FALSE,
                               na.strings = c(""," ","NA"))

# varScanJaspar.file <- 'results/RSAT/randomSeq'
# varScanJasparNeg <- read.delim(file = varScanJaspar.file,
#                                header = TRUE,
#                                sep = "\t",
#                                stringsAsFactors = FALSE,
...
# Split var_coord column
varScanJasparNeg <- separate(data = varScanJasparNeg, 
                             col = "var_coord",
                             into = c("chrom", "pos_Start", "pos_End", "strand"),
                             sep = "[\\:\\-_]", remove = F)

# Rename the motif ID
varScanJasparNeg$X.ac_motif <- gsub(pattern = "_", ".", varScanJasparNeg$X.ac_motif)


varscanColumns <- c("X.ac_motif", "var_id", "var_class", "var_coord", "chrom", "pos_Start", "pos_End", "strand", "best_w", "worst_w", "w_diff", "best_pval", "worst_pval", "pval_ratio", "best_variant", "worst_variant", "best_offset", "worst_offset", "min_offset_diff", "best_strand", "worst_strand", "str_change", "best_seq", "worst_seq", "minor_allele_freq")

varScanJasparNeg <- varScanJasparNeg[, varscanColumns]

varScanJasparNeg$pos_End <- as.numeric(varScanJasparNeg$pos_End)

varScanJasparNeg <- data.frame("chrom" = varScanJasparNeg$chrom,
                            "X.ac_motif" = varScanJasparNeg$X.ac_motif,
                            "var_id" = varScanJasparNeg$var_id,
                            "var_class" = varScanJasparNeg$var_class, 
                            "snpStart" = varScanJasparNeg$pos_End, 
                            "snpEnd" = varScanJasparNeg$pos_End + 1,
                            "strand" = varScanJasparNeg$strand, 
                            "best_w" = varScanJasparNeg$best_w, 
                            "worst_w" = varScanJasparNeg$worst_w, 
                            "w_diff" = varScanJasparNeg$w_diff, 
                            "best_pval" = varScanJasparNeg$best_pval,
                            "worst_pval" = varScanJasparNeg$worst_pval, 
                            "pval_ratio" = varScanJasparNeg$pval_ratio,
                            "best_variant" = varScanJasparNeg$best_variant, 
                            "worst_variant" = varScanJasparNeg$worst_variant, 
                            "best_offset" = varScanJasparNeg$best_offset, 
                            "worst_offset" = varScanJasparNeg$worst_offset,
                            "min_offset_diff" = varScanJasparNeg$min_offset_diff, 
                            "best_strand" = varScanJasparNeg$best_strand,
                            "worst_strand" = varScanJasparNeg$worst_strand, 
                            "str_change" = varScanJasparNeg$str_change, 
                            "best_seq" = varScanJasparNeg$best_seq, 
                            "worst_seq" = varScanJasparNeg$worst_seq, 
                            "minor_allele_freq" = varScanJasparNeg$minor_allele_freq)

# Export the variation-scan results in tsv format
varscan.file <- "varScanJasparNeg.tsv"
varscan.path <- file.path(dir.path["RSAT"], varscan.file)
write.table(x = varScanJasparNeg,
            file = varscan.path,
            quote = FALSE,
            sep = "\t",
            row.names = FALSE,
...
```{r motif_tf}
varScanJasparTF <- merge(x = varScanJaspar, 
                         y = jaspar.core.nr.2018,
                         by.x = "X.ac_motif",
                         by.y = "V1",
                         all.x = TRUE, 
                         all.y = FALSE )
...
```{r varscan_score_distributions, fig.width=8, fig.height=6, out.width="95%", fig.cap = paste(sep="", "**Figure ",(fig.nb <- fig.nb +1),". Distributions of scores returned by variation-scan.** **A.** P-value ratio in positive samples and negative control. **B.** best and worst p-value. **C.** emphazise the differences between  best and worst weight ")}


par(mfrow = c(2,2))

best.pval <- -log10(varScanJaspar$best_pval)
worst.pval <- -log10(varScanJaspar$worst_pval)
best.pval.neg <- -log10(varScanJasparNeg$best_pval)
worst.pval.neg <- -log10(varScanJasparNeg$worst_pval)

pval.range <- range(c(best.pval, worst.pval))

n <- nrow(varScanJaspar)
f <- 1:n / n
...
## Negative control
n <- length(worst.pval.neg)
f <- 1:n / n
lines(sort(best.pval.neg), f, type = "l", col = "blue", lty = "dotted")
lines(sort(worst.pval.neg), f, type = "l", col = "red", lty = "dotted")
...
pvalRatioPos <- log10(varScanJaspar$pval_ratio)
...
## Negative control
n <- length(pvalRatioNeg)
f <- 1:n / n
lines(sort(pvalRatioNeg), f, type = "l", col = "red", lty = "dotted")

grid(col = "#DDDDDD", lty = "solid")

## Weight

best.weight <- varScanJaspar$best_w
worst.weight <- varScanJaspar$worst_w
best.weight.neg <- varScanJasparNeg$best_w
worst.weight.neg <- varScanJasparNeg$worst_w
...
## Negative control
n <- length(worst.weight.neg)
f <- 1:n / n
lines(sort(best.weight.neg), f, type = "l", col = "blue", lty = "dotted")
lines(sort(worst.weight.neg), f, type = "l", col = "red", lty = "dotted")
...
# Make a venn diagram variation-scan inter remap

variationScan <- as.vector(na.omit(remap.inter.varscan.SOIs$TF.x))
remap <- as.vector(na.omit(remap.inter.varscan.SOIs$TF.y)) 

remap.inter.varscanTF <- intersect.Vector(variationScan, remap)

venn.diagram(
  x = list(variationScan, remap),
  category.names = c("variation-scan", "ReMap"),
  filename = "Venn_diagramm.pdf"
)


# Regulatory SNPs
rSNPs <- merge(x = remap.inter.varscan.SOIs,
               y = SOIs,
               by.x = c("snpStart", "snpEnd"),
               by.y = c("pos_hg38", "endpos_hg38"),
               all.x = TRUE, all.y = FALSE)
...
barplot(sort(table(rSNPs$X.ac_motif), decreasing = TRUE), 
        names.arg = "",
...
# Gene
gene <- as.data.frame(table(rSNPs$GENCODE_name))
...
```{r tissueEnrich, fig.width=8, fig.height=6, out.width="95%", fig.cap = paste(sep="", "**Figure ",(fig.nb <- fig.nb +1),". Tissue-specific gene enrichment.** **A. **the x-axis shows each of the tissues, and the y-axis represents the tissue-specific gene enrichment (−Log10(P−Value)) values and the y-axis shows each of the tissues. The  tissue-specific gene show enrichment for colon, heart muscle, lung, stomach, urinary bladder, esophagus, skeletal muscle and small intestinal specific genes. **B. ** tissue-specific gene enrichment using the fold change.")}

inputGenes <- unique(na.omit(rSNPs$GENCODE_name))
...
## ggplot
A <- ggplot(enrichmentOutput,aes(x=reorder(Tissue,-Log10PValue),y=Log10PValue,label = Tissue.Specific.Genes,fill = Tissue))+
...
seExp<-output[[2]][["Stomach"]]
seExp <- enrichmentOutput
